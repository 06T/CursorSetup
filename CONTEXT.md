# 🧠 AI-Optimized Project Context: Cursor Agent Workspace Template

## 1. Executive Summary & Core Mission

**Project Name:** Cursor Agent Workspace Template
**Core Technology:** OpenAI-compatible APIs (GPT-4o, GPT-4 Turbo, etc.) optimized for Cursor IDE
**Mission:** To provide a "Zero-Config," enterprise-grade starter kit for building autonomous AI agents. The primary goal is to clone the repository, open it in Cursor IDE, and have the AI agent immediately understand its purpose, architecture, and operational protocols without manual prompting.
...
## 2. Cognitive Architecture & Agent Persona (`.cursor/rules.md`)

This is the agent's "brain" or "constitution." It dictates the agent's behavior, personality, and constraints.

*   **Persona:** The AI MUST act as a **"Cursor Expert"**—a Senior Developer Advocate and Solutions Architect. It is knowledgeable, professional, and follows best practices.
...
*   **Infinite Memory Engine (`src/memory.py`):** A JSON-based memory manager that uses recursive summarization to manage conversation history, ensuring context is never lost. This is a core feature for context compression and management.
...
*   **Tech Stack:**
    *   `openai`: The official OpenAI API client (compatible with Cursor's models).
    *   `pydantic`: For robust data modeling.
    *   `python-dotenv`: For managing environment variables (e.g., `OPENAI_API_KEY`).
*   **DevOps:**
...
*   **Key Directories:**
    *   `.cursor/`: Core AI rules and persona. **(Crucial for agent behavior)**.
...
1.  **Understand Your Role:** You are a Cursor Expert. Your primary directive is to build upon this existing framework.
...
4.  **Follow the Rules:** Adhere strictly to the coding standards and behavioral protocols defined in `.cursor/rules.md` and `.context/`.
